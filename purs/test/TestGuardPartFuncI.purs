module Test.TestGuardPartFuncI where
...
main :: Effect Unit
main = do
  log "=== parseGuardPart function test I ==="
            ...
            Just t | t.tokenType == TokIdentifier -> do
              ...
              case P.expectOperator (Array.drop 1 rest') "::" of
                Right (Tuple _ rest2) -> do
                  ...
                  log ""
                  log "parseFunctionDeclarationRaw on rest3:"
                  case P.parseFunctionDeclarationRaw rest3 of
                    Right (Tuple fun restFinal) -> do
                      log $ "✓ " <> fun.name
                      log $ "Rest: " <> show (map showTok (Array.take 5 restFinal))
                    Left err -> log $ "✗ " <> err
