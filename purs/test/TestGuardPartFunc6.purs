module Test.TestGuardPartFunc6 where
...
main :: Effect Unit
main = do
  log "=== parseGuardPart function test 6 ==="
    ...
    _ -> case P.parseDeclaration tokens' of
      Right (Tuple decl rest) -> do
        if n == 83 then do
          ...
          log $ "funcToks first 5: " <> show (map showTok (Array.take 5 funcToks))
          
          -- Try parseIdentifierName directly
          log ""
          log "Trying parseIdentifierName on funcToks:"
          case P.parseIdentifierName funcToks of
            Right (Tuple name rest2) -> do
              log $ "✓ Got name: " <> name
