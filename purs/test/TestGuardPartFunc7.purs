module Test.TestGuardPartFunc7 where
...
main :: Effect Unit
main = do
  log "=== parseGuardPart function test 7 ==="
      ...
      Right (Tuple decl rest) -> do
        if n == 83 then do
          ...
          case P.parseIdentifierName funcToks of
            Right (Tuple name rest1) -> do
              log $ "  ✓ name = " <> name
              log $ "  rest1: " <> show (map showTok (Array.take 5 rest1))
              
              log "Step 2: parseMany parseSimplePattern"
              case P.parseMany P.parseSimplePattern rest1 of
                Right (Tuple params rest2) -> do
                  log $ "  ✓ params count = " <> show (Array.length params)
                  log $ "  rest2: " <> show (map showTok (Array.take 5 rest2))
