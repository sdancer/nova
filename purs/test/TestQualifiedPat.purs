module Test.TestQualifiedPat where
...
main :: Effect Unit
main = do
  log "=== Qualified Constructor Pattern Test ==="
  
  let input = "Ast.PatVar v"
  ...
  log ""
  log "parseConstructorPattern:"
  case P.parseConstructorPattern tokens of
    Right (Tuple pat rest) -> do
      log "✓ parsed"
      log $ "Rest: " <> show (map showTok rest)
    Left err -> log $ "✗ " <> err
