module Test.TestParseFuncBody2 where
...
main :: Effect Unit
main = do
  log "=== Parse Function Body Test 2 ==="
  ...
  -- Try parsing as function declaration
  ...
  -- Try parsing just the expression
  log ""
  log "parseExpression on body tokens:"
  let bodyTokens = Array.drop 3 tokens  -- skip "parseGuardExpression tokens ="
  case P.parseExpression bodyTokens of
    Right (Tuple expr rest) -> do
      log "✓ Success"
      let rest' = P.skipNewlines rest
      log $ "Rest: " <> show (map showTok (Array.take 15 rest'))
    Left err -> log $ "✗ " <> err
