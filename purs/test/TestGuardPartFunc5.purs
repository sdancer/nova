module Test.TestGuardPartFunc5 where
...
main :: Effect Unit
main = do
  log "=== parseGuardPart function test 5 ==="
    ...
    _ -> case P.parseDeclaration tokens' of
      Right (Tuple decl rest) -> do
        if n == 83 then do
          ...
          -- Try parsing the rest with parseFunctionDeclarationRaw directly
          let toks = P.dropNewlines rest
          -- Skip to the function definition (after the type sig)
          let Tuple _ funcToks = P.splitTypeAndRest (Array.drop 2 toks) "parseGuardPart"  -- Skip name and ::
          log $ "funcToks: " <> show (map showTok (Array.take 15 funcToks))
          
          log ""
          log "Trying parseFunctionDeclarationRaw on funcToks:"
          case P.parseFunctionDeclarationRaw funcToks of
            Right (Tuple fun rest2) -> do
              log $ "✓ Got function: " <> fun.name
              log $ "Rest: " <> show (map showTok (Array.take 5 rest2))
            Left err -> log $ "✗ " <> err
        else pure unit
        if n < 90 then
